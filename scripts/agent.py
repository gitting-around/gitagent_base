...
class Agent:
	...
	def adapt(self):
		...
		if self.attempted_jobs_depend == 0:
			rate_depend = 0.0
		else:
			rate_depend = 1.0 * self.completed_jobs_depend / self.attempted_jobs_depend

		accept = self.about_me_wm.keep_request(self.current_client[self.client_index], self.active_servs[self.task_idx], self.old_state, self.service, self.stdout_log, rate, rate_depend)
		if self.handle > 0:
			...
			if not len(self.about_me_wm.known_people) == 0:
				...
				if anyone:
					...
					try:
						...
						self.write_log_file(self.stdout_log, '[run_step ' + str(self.run_step) + '] it went to heell ' + str(server_resp) + '\n')
						self.it_went_to_hell = self.it_went_to_hell + 1
						server_resp = -1
						pass
...
if __name__ == '__main__':
	...
	#params from 2 to last 0.5 redundant -- should remove -- won't need in this format probably
	...
	try:
		...
		agent.write_log_file(filename, str(agent.about_me_wm.ID) + '	' + str(agent.timeouts) + '	' +str(agent.handle)+'	'+ str(agent.depend_myself) + '	'+str(agent.completed_jobs_depend) +'	'+str(agent.attempted_jobs_depend) + '	' + str(agent.completed_jobs) + '	' + str(agent.attempted_jobs) +'	'+ str(agent.collected_reward) + '	' + str(agent.count_loops) + '	'+str(dur)+'	'+str(agent.COUNT_noones)+'	'+str(agent.count_posReq)+'	'+str(agent.conn_reset)+'	'+str(agent.it_went_to_hell) + '\n')
		...
		agent.write_log_file(filename4, str(agent.about_me_wm.ID) + '	')
		for x in agent.about_me_wm.moving_depend_done:
			agent.write_log_file(filename4, str(x) + '	')
		agent.write_log_file(filename4, '\n')
