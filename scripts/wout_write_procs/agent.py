...
class Agent:
	...
	def __init__(self, delta, energy, ID, x, y, theta, red, green, blue, state, depend_nr, popSize, provaNr):
		...
		self.stdout_log.write('[INIT] active_servs: ' + str(self.active_servs) + '\n')
		self.service = []
	...
	def call_serve(self, server, myid, request, anyone_index):
		...
		try:
			...
			self.stdout_log.write('[call_serve ' + str(self.call) + '] ' + 'perceived willingness from server: %d, is: %f\n' % (server, self.about_me_wm.known_people[anyone_index][1]))
			...
			self.stdout_log.write('[call_serve ' + str(self.call) + '] ' + 'perceived expertise from server: %d, is: %f\n' % (server, self.about_me_wm.known_people[anyone_index][3][service_idx]))

			self.stdout_log.write('[call_serve ' + str(self.call) + '] ' + 'capability expertise: ' + str(self.capability_expertise) + '\n')
	...
	def callback(self, data):
		...
		self.stdout_log_callback.write('[callback ' + str(self.callback_bc) + '] capability_expertise ' + str(self.capability_expertise) + '\n')
	###################################################################################################
	...
	def run_fsm(self):
		while not rospy.is_shutdown():
			...
			if time.time() > self.TIME + self.DURATION:
				...
				self.stdout_log.write('[fsm ' + str(self.fsm) + '] Finished Correctly\n')
				return

			self.fsm = self.fsm + 1
			if True in self.adaptive_state:
				self.stdout_log.write('[fsm ' + str(self.fsm) + '] current client_index: ' + str(self.client_index) + '\n')

				self.old_client_index = self.client_index

				self.client_index = self.adaptive_state.index(True)

				self.stdout_log.write('[fsm ' + str(self.fsm) + '] ' + str(self.adaptive_state) + ' ' + str(self.current_client) + '\n')
	...
	def wander(self):
		self.w = self.w + 1
		print 'wander'
		self.stdout_log.write('[wander ' + str(self.w) + '] wander\n')
		if random.random() > 0.1:
			if not rospy.is_shutdown():
				self.publish_global.publish(self.message)
				print 'Published'
				self.stdout_log.write('[wander ' + str(self.w) + '] published\n')
		if random.random() > 0.7:
			...
			self.stdout_log.write('[wander ' + str(self.w) + '] Chosen service: ' + str(self.service) + '\n')

			self.about_me_wm.state = 2
		else:
			print 'do nothing - zot jepi atij qe rri kot :DD'
			self.stdout_log.write('[wander ' + str(self.w) + '] do nothing - zot jepi atij qe rri kot :DD\n')
	...
	def adapt(self):
		self.a = self.a + 1
		print 'adapting'
		self.stdout_log.write('[adapt ' + str(self.a) + '] adapting\n')
		...
		self.stdout_log.write('[adapt ' + str(self.a) + '] client: %d, service_resp: %s\n' % (self.current_client[self.client_index], str(self.service_resp[self.client_index])))

		self.stdout_log.write('[adapt ' + str(self.a) + '] old serve: %s\n' % (str(self.service)))
		...
		accept = self.about_me_wm.keep_request(self.current_client[self.client_index], self.active_servs[self.task_idx], self.old_state, self.service, self.stdout_log)

		print 'accept ' + str(accept)
		self.stdout_log.write('[adapt ' + str(self.a) + '] accept ' + str(accept) + '\n')

		if accept:
			print 'adapted'
			self.count_posReq = self.count_posReq + 1
			self.stdout_log.write('[adapt ' + str(self.a) + '] adapted\n')
			...
			self.helping = True
			print self.service
			self.stdout_log.write('[adapt ' + str(self.a) + '] service: ' + str(self.service) + '\n')
			self.stdout_log.write('[adapt ' + str(self.a) + '] helping: ' + str(self.helping) + '\n')

		else:
			print 'keep at what you\'re doing'
			#praktikisht e le pergjysem kerkesen
			self.stdout_log.write('[adapt ' + str(self.a) + '] keep at what you\'re doing\n')
			self.stdout_log.write('[adapt ' + str(self.a) + '] before if: current client_index: ' + str(self.client_index) + '\n')
			...
			self.stdout_log.write('[adapt ' + str(self.a) + '] after old state: current client_index: ' + str(self.client_index) + '\n')
...
#rregullo kete qe te mos quaje te bere ato jobs qe kane deshtuar
		if i <= service[1] and not depend:
			...
			self.stdout_log.write('[run_step ' + str(self.run_step) + '] Doing task %d, iteration: %d\n' % (service[0], i))
			...
			if self.iteration > service[1]:
				...
				self.iteration = 1
				self.stdout_log.write('[run_step ' + str(self.run_step) + '] client_index %d, helping %s\n' % (self.client_index, self.helping))
				if self.helping:
					self.service_req[self.client_index] = -1
					if random.random() > 0: #it will always succeed
						...
						self.stdout_log.write('[run_step ' + str(self.run_step) + '] Helping, Task %d, done, wandering again\n' % service[0])
					else:
						self.service_resp_content[self.client_index] = 0
						print 'Helping, Task %d, failed, wandering again' % service[0]
						self.stdout_log.write('[run_step ' + str(self.run_step) + '] Helping, Task %d, failed, wandering again\n' % service[0])
					self.service_resp[self.client_index] = True
					self.helping = False
					self.client_index = -1
				else:
					...
					self.stdout_log.write('[run_step ' + str(self.run_step) + '] Task %d, done, wandering again\n' % service[0])

		elif i <= service[1] and depend:
			print 'Doing task %d, iteration. Depend is true' % service[0]
			self.stdout_log.write('[run_step ' + str(self.run_step) + '] Doing task %d, iteration. Depend is true\n' % service[0])
			...
			if not len(self.about_me_wm.known_people) == 0:
				...
				self.stdout_log.write('[run_step ' + str(self.run_step) + '] anyone_id: ' + str(anyone_id) + '\n')
				self.stdout_log.write('[run_step ' + str(self.run_step) + '] anyone_id_idx: ' + str(anyone_id_idx) + '\n')
				self.stdout_log.write('[run_step ' + str(self.run_step) + '] subset: ' + str(subset_known) + '\n')
				if anyone:
					...
					if self.run_step < self.steps_b4_equilibrium:
						...
						self.stdout_log.write('[run_step ' + str(self.run_step) + '] jj at random: %d\n' % jj)
					else:
						if random.random() < 0.8:
							jj = subset_known.index(max(subset_known, key=lambda x: x[1]))
							self.stdout_log.write('[run_step ' + str(self.run_step) + '] jj using lambda function: %d\n' % jj)
						else:
							jj = random.randint(0, len(anyone_id) - 1)
							self.stdout_log.write('[run_step ' + str(self.run_step) + '] jj at random after equilibrium: %d\n' % jj)
					anyone_index = anyone_id_idx[jj]
					server = anyone_id[jj]
					self.stdout_log.write('[run_step ' + str(self.run_step) + '] anyone_id: ' + str(anyone_id[jj]) + '\n')
					self.stdout_log.write('[run_step ' + str(self.run_step) + '] anyone_index: ' + str(anyone_index) + '\n')
					print server
					self.stdout_log.write('[run_step ' + str(self.run_step) + '] server: ' + str(server) + '\n')
					...
					self.stdout_log.write('[run_step ' + str(self.run_step) + '] checking after service return: ' + str(server_resp) + '\n')
					self.about_me_wm.state = 1
					self.iteration = 1

					if server == self.client_index:
						self.count_loops = self.count_loops + 1
						self.stdout_log.write('[run_step ' + str(self.run_step) + '] counting loops + 1' + '\n')
					...
					if server_resp == 1:
						...
						prob = random.random()
						self.stdout_log.write('[run_step ' + str(self.run_step) + '] prob %d: \n' % prob)
						if prob <= 0.3:
							self.completed_jobs = self.completed_jobs + 1
							self.completed_jobs_depend = self.completed_jobs_depend + 1
							self.collected_reward = self.collected_reward + service[3]
							...
							print 'Task %d, done, wandering again' % service[0]
							self.stdout_log.write('[run_step ' + str(self.run_step) + '] Task %d, done by myslef, after failed help request\n' % service[0])
							server_resp = 1
						else:
							print 'Task %d, failed, wandering again' % service[0]
							self.stdout_log.write('[run_step ' + str(self.run_step) + '] Task %d, failed, both help request and by myself\n' % service[0])
					if self.helping:
						self.service_req[self.client_index] = -1
					...
					self.stdout_log.write('[run_step ' + str(self.run_step) + '] no one to ask that has this capability\n')
					self.stdout_log.write('[run_step ' + str(self.run_step) + '] Doing task %d\n' % service[0])
				...
				print 'no one to ask'
				self.COUNT_noones = self.COUNT_noones + 1
				self.stdout_log.write('[run_step ' + str(self.run_step) + '] no one to ask\n')
				self.stdout_log.write('[run_step ' + str(self.run_step) + '] Doing task %d\n' % service[0])
...
if __name__ == '__main__':
	...
	try:
		...
		agent.stdout_log_callback.close()
		agent.stdout_log_handle.close()

		dur = time.time() - starttime

		filename = 'RESULT/pop_size.'+str(popSize) +'/prova.'+str(provaNr)+'/reward_results_' + str(agent.about_me_wm.delta) +'_'+ str(depends) + '_'+ str(agent.about_me_wm.ID)
 
		with open(filename, 'a') as results_log:
			fcntl.flock(results_log, fcntl.LOCK_EX)
			results_log.write(str(agent.about_me_wm.ID) + '	' + str(agent.timeouts) + '	' +str(agent.handle)+'	'+ str(agent.depend_myself) + '	'+str(agent.completed_jobs_depend) +'	'+str(agent.attempted_jobs_depend) + '	' + str(agent.completed_jobs) + '	' + str(agent.attempted_jobs) +'	'+ str(agent.collected_reward) + '	' + str(agent.count_loops) + '	'+str(dur)+'	'+str(agent.COUNT_noones)+'	'+str(agent.count_posReq)+'\n')

			fcntl.flock(results_log, fcntl.LOCK_UN)

		filename1 = 'RESULT/pop_size.'+str(popSize) +'/prova.'+str(provaNr)+'/moving-delta_results_' + str(agent.about_me_wm.delta) +'_'+ str(depends) + '_'+ str(agent.about_me_wm.ID)
		with open(filename1, 'a') as results_log1:
			fcntl.flock(results_log1, fcntl.LOCK_EX)
			results_log1.write(str(agent.about_me_wm.ID) + '\n')
			for x in agent.about_me_wm.moving_delta:
				results_log1.write(str(x) + '\n')

			results_log1.write('\n\n')
			fcntl.flock(results_log1, fcntl.LOCK_UN)

		filename2 = 'RESULT/pop_size.'+str(popSize) +'/prova.'+str(provaNr)+'/cap-expertise_results_' + str(agent.about_me_wm.delta) +'_'+ str(depends) + '_'+ str(agent.about_me_wm.ID)
		with open(filename2, 'a') as results_log2:
			fcntl.flock(results_log2, fcntl.LOCK_EX)
			results_log2.write(str(agent.about_me_wm.ID) + '\n')
			for x in agent.about_me_wm.known_people:
				results_log2.write('	' + str(x) + '\n')
				results_log2.write('	' + str(agent.capability_expertise[agent.about_me_wm.known_people.index(x)]) + '\n\n')

			results_log2.write('\n\n')
			fcntl.flock(results_log2, fcntl.LOCK_UN)
