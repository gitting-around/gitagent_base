	...
	def handle_serve(self, request):
		...
		while not self.service_resp[idx]:
			...
			if time.time() > timeout:
				...
				self.lock.acquire()
				self.write_log_file(self.stdout_handle, '[handle_serve ' + str(local_handle) + '] timeout\n')
				self.lock.release()
				break
		...
		self.lock.acquire()
		self.write_log_file(self.stdout_handle, '[handle_serve ' + str(local_handle) + '] request outgoing ' + outgoing + '\n')
		self.lock.release()
	...
	def wander(self):
		...
		if random.random() > 0.1:
			if not rospy.is_shutdown():
				...
				print 'Published'
				self.write_log_file(self.stdout_log, '[wander ' + str(self.w) + '] published\n')
